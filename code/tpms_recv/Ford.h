  ...
  #define CC1101_DEFVAL_PKTCTRL0   0x12        // Packet Automation Control - synchronous data
...
void ConvertTimingsToBits() {
  ...
  for(int16_t i = 0; i <= TimingsIndex; i++) {
    if(IsValidShort(Timings[i])) {
      ...
      if(IsValidLong(Timings[i])) {
        ...
        if(IsTooShort(Timings[i]) || IsTooLong(Timings[i])) { //enough bits in the buffer to continue trying?
          if(i < TimingsIndex/2) { //invalid bit timing, reset
            BitCount = 0;
          } else { // end the conversion
            //assume an end bit
...
bool ValidateTimings() {
  ...
  if(TimingsIndex < (SYNCBITS + EXPECTEDBITCOUNT) ) { //not enough in the buffer to consider a valid message
    #ifdef SHOWDEBUGINFO
  ...
  if(TimingsIndex > 200) { //not enough in the buffer to consider a valid message
    #ifdef SHOWDEBUGINFO
